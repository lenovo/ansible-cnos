...
version_added: "1.7"
module: cnos_factory
short_description: Resets start up config to factory settings
description: 
    - Manages network device configurations over SSH.
    This module allows implementors to work with the device configurations. This method provides a way you 
    can reset the start up configuration of the device tro factory settings. So that you can move the device 
    to another topology as a fresh piece. Results of the resetting to factory settings  operation can be viewed
    in results folder.
...
- name: Test Reset to factory
  cnos_factory:  host={{ inventory_hostname }} username={{ hostvars[inventory_hostname]['username']}}  password={{ hostvars[inventory_hostname]['password']}} deviceType={{ hostvars[inventory_hostname]['deviceType']}} outputfile=./results/demo_template_command_{{ inventory_hostname }}_output.txt 
  with_items: "{{test_save_data2}}"
...
On successful execution, the method returns and empty string with a message "Switch is reset to factory 
settings" in json format. But upon any failure, the output will be the error display string. You may have to 
