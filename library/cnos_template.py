...
short_description: Manage Lenovo CNOS device configurations using Template
description: 
    - Manages network device configurations over SSH. This module allows implementors to work with the device 
     ...
     can be a set of commands or a template written in Jinja2 format.
options:
...
- name: Replace Config CLI command template with values
  template: src=demo_template.j2 dest=./commands/demo_template_{{ inventory_hostname }}_commands.txt
  with_items: "{{demo_template_data}}"

- name: Applying CLI commands on Switches
  cnos_template: host={{ inventory_hostname }} username={{ hostvars[inventory_hostname]['username']}}  
    ...
    with open(commandfile, "r") as f:
        for line in f:
            ...
            if not line.startswith("#"):
                output = output + cnos_utility.waitForDeviceResponse(line,"#", 2, remote_conn)
